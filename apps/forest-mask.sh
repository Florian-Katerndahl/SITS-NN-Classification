...
# TODO Don't hardcode blocksizes in last step
# Should be incorporated into NF workflow!
set -e
...
for ((i=LOWER_LATITUDE; i<=UPPER_LATITUDE; i+=3)); do
  for ((j=LOWER_LONGITUDE; j<=UPPER_LONGITUDE; j+=3)); do
    #! THIS ONLY WORKS FOR THE NOTHERN HEMISPHERE, EAST OF GREENWICH => fixed in groovy closure
...
find Eolab/masks -name '*tif' -type f -execdir \
  ...
  --creation-option="COMPRESS=LZW" --creation-option="PREDICTOR=2" --overwrite -A {} +
